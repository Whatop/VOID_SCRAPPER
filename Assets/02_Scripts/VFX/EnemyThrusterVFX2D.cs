...
/// <summary>
/// 적 기체의 배기구 Transform에서 이동 중 배기 VFX를 생성한다.
/// 일반 AI 이동과 화면 밖 진입 연출처럼 Transform 기반 이동도 감지한다.
/// </summary>
[DisallowMultipleComponent]
public class EnemyThrusterVFX2D : MonoBehaviour
{
    [Header("References")]
    [SerializeField] private EnemyBaseAI enemyAI;
    [SerializeField] private EnemyHealth enemyHealth;
    [SerializeField] private Rigidbody2D body;
    [Tooltip("실제 물리 이동 Root를 연결하세요. VisualRoot는 부유 애니메이션을 이동으로 오인할 수 있습니다.")]
    [SerializeField] private Transform movementReference;

    [Header("Exhaust Points")]
    [Tooltip("기체 후방 배기구 위치 Transform들입니다.")]
    ...
    [Header("Spawn Timing")]
    [Min(0.01f)]
    [SerializeField] private float moveSpawnInterval = 0.09f;
    [Min(0.01f)]
    ...
    [SerializeField] private bool useAiDesiredVelocity = true;
    ...
    [SerializeField] private float positionJitter = 0.015f;
    ...
    private Vector2 ResolveMovementVelocity()
    {
        ...
        if (useAiDesiredVelocity &&
            enemyAI != null &&
            enemyAI.DesiredVelocity.sqrMagnitude > bestVelocity.sqrMagnitude)
        {
            bestVelocity = enemyAI.DesiredVelocity;
        }

        if (useMeasuredTransformMovement &&
            measuredVelocity.sqrMagnitude > bestVelocity.sqrMagnitude)
        {
    ...
    private void SpawnExhaust(GameObject prefab)
    {
        ...
        if (spawnAllExhaustPoints)
        {
            ...
            {
                if (exhaustPoints[i] != null)
                {
                    SpawnAtPoint(exhaustPoints[i], prefab);
                }
            }
    ...
    private void SpawnAtPoint(Transform point, GameObject prefab)
    {
        ...
        Quaternion rotation = ResolveEffectRotation(point);
        GameObject instance;
    ...
    private Quaternion ResolveEffectRotation(Transform point)
    {
